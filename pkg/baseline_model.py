...
class SurvBasic(torch.nn.Module):
    ...
    def _set_background(self, X: torch.Tensor, T: torch.Tensor, D: torch.Tensor):
        ...
        self.Z_bg = self.nn_model(X[sort_args])
    ...
    def fit(self, X: np.ndarray, y: np.recarray, 
        ...
        for e in range(1, self.epochs + 1):
            ...
            data = dataset_generator(X, t, d, bg_size, self.batch_num)
            ...
            for x_b, t_b, d_b, x_t, t_t, d_t in prog_bar:
                ...
                z_t = self.nn_model(x_t)
                
                target_ds = TensorDataset(z_t, t_t, d_t)
                target_loader = DataLoader(target_ds, sub_batch_len, False)
                surv_loss = 0

                for z_t_b, t_t_b, d_t_b in target_loader:
                    z_t_b, t_t_b, d_t_b = z_t_b.to(self.device), t_t_b.to(self.device), d_t_b.to(self.device)
